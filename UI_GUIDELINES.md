...
管理端改为“客户队列”，每位客户一个入口，显示最近消息、全部咨询待办汇总和点击后将打开的咨询状态。默认入口优先待审批和人工接管；状态筛选或搜索时只在匹配咨询中选择入口，汇总仍明确标注为全部咨询。

详情头部下方复用原生 details 和 Link，提供进行中咨询切换及默认收起的“查看历史咨询”，每页 10 条。选中的历史咨询标为“历史咨询 · 已解决”，上下文、审批、人工回复继续绑定该咨询。切换历史时客户行仍高亮。原有四栏结构、样式变量和同步组件保持；不创建额外 UI 依赖，不引入未读标记。
