...
export async function sendCustomerMessage(customerId: string, input: Record<string, unknown>) {
  ...
  const header = typeof input.conversationId === "string" ? await ownedConversation(customerId, input.conversationId) : null;
  const human = header?.status === "human_handoff";
  let content = input.content.trim();
  if (!human && /退款|refund/i.test(content) && !/人工|human/i.test(content)) {
  ...
  if (typeof input.conversationId === "string") {
    ...
    await prisma.$transaction(async tx => {
      if (!(await tx.conversation.updateMany({ where: { id, customerId, status: human ? "human_handoff" : "open", executions: { none: { status: "running" } } }, data: { updatedAt: new Date() } })).count) throw new RuntimeConflictError("会话当前不能发送消息。");
