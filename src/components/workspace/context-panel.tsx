...
export async function ContextPanel({ conversation, page, executionPage, filters = {}, approvals = [] }: { conversation: ConversationHeader; page: ExecutionPage; executionPage: number; filters?: InboxFilters; approvals?: Approval[] }) {
  ...
  return <div className="context-content">
    ...
    <section className="context-section" aria-label="AI 协作"><h3>AI 协作</h3><AIStatus status={conversation.status} />{latest ? <><p className="execution-scope">本页最近执行 · {formatDate(latest.createdAt)}<br />记录快照 · 刷新页面获取最新结果</p><ActivityTimeline steps={steps} /></> : <p className="muted">暂无执行记录</p>}<p className="muted">依据与引用 · 暂无已记录的知识来源</p></section>
