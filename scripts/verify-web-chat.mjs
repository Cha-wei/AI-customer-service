...
try {
  ...
  for (let i = 0; i < 24; i++) await client.conversation.create({ data: { customerId: 'queue-sync-' + i, status: 'open', updatedAt: new Date(queueBase + i), messages: { create: { role: 'customer', content: '队列同步测试 ' + i } } } });
  await expect(staff.locator('.conversation-row')).toHaveCount(20, { timeout: 15000 });
  ...
  await client.conversation.create({ data: { customerId: 'queue-sync-new', status: 'open', messages: { create: { role: 'customer', content: '新到会话' } } } });
  await expect(staff.locator('.conversation-row').first()).toContainText('queue-sync-new', { timeout: 15000 });
  await expect(staff.getByRole('searchbox')).toHaveValue('尚未提交的搜索');
  assert.equal(await queueList.evaluate(el => el.scrollTop), 240, 'queue refresh preserves scroll offset');
  assert.equal(new URL(staff.url()).searchParams.get('status'), 'open');
  assert.equal(new URL(staff.url()).searchParams.get('query'), 'queue-sync');
  assert.equal(new URL(staff.url()).searchParams.get('page'), '1');
  await client.conversation.updateMany({ where: { customerId: 'queue-sync-new' }, data: { status: 'resolved' } });
  await expect(staff.locator('.conversation-row').filter({ hasText: 'queue-sync-new' })).toHaveCount(0, { timeout: 15000 });
