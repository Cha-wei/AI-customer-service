...
管理端的 `customer-queue-reader` 是独立只读投影：数据库先按客户分组分页，再读取当前页客户的汇总与目标咨询；原 Conversation Repository、客户 API 和所有写操作继续使用独立 conversationId。历史咨询按客户隔离、单独分页。详情同步摘要同时包含该客户咨询的状态数量和最近更新时间，使客户不在当前队列筛选页时也能更新咨询切换入口。
出现多个方案时，优先级为：
